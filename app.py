...
from Db_Operations import get_all_bookings
app = Flask(__name__)
...
@app.route('/Agentloginhtml',methods=['POST','GET'])
def Agentloginhtml():
...
def agentdashboard():
    ...
    return render_template('agentdashboard.html',booking_data=booking_data)

@app.route('/Carbooking', methods=['GET', 'POST'])
def Carbooking():
    if request.method == 'POST':
        car_model = request.form.get('carModel')
        payment_info = request.form.get('paymentInfo')
        model_number = request.form.get('modelNumber')
        appointment_date = request.form.get('appointmentDate')
 
        cursor = mysql.cursor()
        cursor.execute('''
            INSERT INTO car_bookings (car_model, payment_info, model_number, appointment_date)
            VALUES (%s, %s, %s, %s)
        ''', (car_model, payment_info, model_number, appointment_date))
        mysql.commit()
        cursor.close()
...
def agentlogin():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        if account:
            if account[7] != 'agent':
                return render_template('Custlogin.html', alert="Please login as a Customer to access this page!")
            if (username == account[3] and password == account[6]) and account[7]=='agent' :
                msg = 'Logged in successfully !'
