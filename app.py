...
def edit_user(id):
    ...
    if request.method == 'GET':
        ...
        grocery = cursor.fetchone()
        cursor.close()
        print("accounts",grocery[7])
        print("accounts",grocery[6])
        
        return render_template('edit_user.html', grocery=grocery)
    
    elif request.method == 'POST':
        # Update the grocery service booking based on the provided ID
        cursor.execute("""UPDATE accounts
            SET fname=%s, email=%s  WHERE id = %s""", (request.form['fname'], request.form['email'], id))
        mysql.commit()
        cursor.close()
